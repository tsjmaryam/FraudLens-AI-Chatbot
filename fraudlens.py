...
def behavioral_narrative(behavior: dict) -> str:
    ...
    amount_txt = f"${float(amount):,.2f}" if amount else "an unusual amount"
...
def shap_plain_language(contribs: list) -> str:
    if not contribs: return "No key risk drivers identified."
    lines = []
    for c in contribs[:3]:
        f = c.get("feature","feature").replace("_"," ").title()
        v = c.get("value","?")
        pct = c.get("pct_of_total",0)*100
        direction = "increased" if c.get("contribution",0)>0 else "decreased"
        lines.append(f"{f} = {v} ({direction} risk, ~{pct:.0f}% influence)")
    return "; ".join(lines) + "."

# =========================================================
# --- GPT Narrative (Analyst Style)
# =========================================================
...
def gpt_narrate(pred: int, proba: float, threshold: float, contribs, features: Dict[str, Any]) -> str:
    """Generate a unified investigative narrative combining EBM, SHAP, and behavior anomalies."""
    if not USE_GPT:
        return ""

    decision = "FRAUD" if pred == 1 else "NOT FRAUD"
    score_pct = round(proba * 100, 1)
    cutoff_pct = round(threshold * 100, 1)
    behavior = build_behavior_dict(features)
    behavior_text = behavioral_narrative(behavior)
    shap_summary = shap_plain_language(contribs)

    case_facts = {
        "decision": decision,
        "score_percent": score_pct,
        "threshold_percent": cutoff_pct,
        "behavior_summary": behavior_text,
        "key_factors": contribs,
        "plain_summary": shap_summary,
        "features": features or {}
    }
    case_facts = _to_jsonable(case_facts)

    system_prompt = (
        "You are an experienced financial fraud investigator preparing a concise internal case summary. "
        "Avoid markdown, bullets, or headings. Write in analytical prose, similar to a SAR narrative.\n\n"
        "Paragraph 1: Summarize the model's decision and confidence, referencing that the Explainable Boosting Machine (EBM) "
        "flagged the case based on elevated anomaly likelihood.\n"
        "Paragraph 2: Describe the behavioral anomalies—time, location, merchant type, chip use, and amount—and why they deviate "
        "from the customer's normal activity or peer profile.\n"
        "Paragraph 3: Integrate SHAP/EBM features explaining which attributes contributed to the alert and why they indicate "
        "possible fraudulent intent or account compromise. Conclude with a professional judgment statement such as "
        "'These indicators justify the model's alert and require further analyst review.'"
    )
...
# =========================================================
# --- Display Function: Narrative + SHAP Plot
# =========================================================

def display_case_with_waterfall(
    txn_id: str,
    df_row: pd.DataFrame,
    ebm_score: float,
    pred: int,
    ...
    """Full Analyst View: Generates GPT narrative and SHAP plot below it."""
    contribs = shap_top_contribs_row(df_row, topn=5)
    features = df_row.iloc[0].to_dict()

    # Try GPT; fallback to unified
    try:
        narrative_text = gpt_narrate(pred, ebm_score, threshold, contribs, features)
    except Exception:
        from unified_explanation import unified_explanation
        narrative_text = unified_explanation(txn_id, ebm_score, contribs, features)

    st.markdown("### 🧾 Case Narrative")
    st.markdown(narrative_text)

    shap_png_b64 = shap_waterfall_png_for_row(df_row)
    if shap_png_b64:
        st.markdown("### 📊 Feature Impact (SHAP Waterfall)")
        st.image(f"data:image/png;base64,{shap_png_b64}", use_column_width=True)
    else:
        st.warning("No SHAP waterfall plot available for this transaction.")

# =========================================================
# --- Example Usage in Streamlit
# =========================================================

# Simulate a transaction (replace with your actual selection logic)
if st.button("Generate Explanation for Demo Case"):
    if not DB.empty:
        txn_row = DB.sample(1, random_state=42)
        proba = MODEL.predict_proba(txn_row)[:, 1]
        pred = (proba >= BEST_TH).astype(int)
        display_case_with_waterfall(
            txn_id="TXN-DEMO-001",
            df_row=txn_row,
            ebm_score=float(proba[0]),
            pred=int(pred[0]),
            threshold=BEST_TH
        )
...
def gpt_extract_intent_and_fields(text: str) -> Dict[str, Any]:
    """
    Use GPT to analyze the user's message and extract structured intent & fields.
    Returns a dictionary like:
      {
        "intent": "lookup" | "score_single" | "batch" | "unknown",
        "filters": {...},                
        "fields": {                      
           "amount": 3000.0,
           "merchant_state": "DC",
           "merchant_city": "Washington",
           "merchant_type": "Grocery Stores, Supermarkets",
           "use_chip": "Unknown"
        },
        "need_example": false,           
    ...
    sys = (
        "You are a structured intent extractor for a financial fraud detection chatbot. "
        "Return ONLY valid JSON with these keys: intent, filters, fields, need_example, example_type, confidence. "
        "Infer the user's intent among: "
        "lookup (find an existing transaction), "
        "score_single (describe a new or hypothetical transaction to evaluate), "
        "batch (mentions CSV/upload), or unknown. "
        "Detect any question about fraud, risk, suspicious activity, or alerts as 'score_single'. "
        "Add a numeric confidence between 0 and 1 indicating certainty. "
        "Normalize fields into model-ready variables (amount, merchant_state, merchant_city, merchant_type, use_chip, etc.)."
    )

    user = f"User input:\n{text}\n\nReturn a single JSON object exactly as requested."
    ...
    try:
        ...
        try:
            ...
            return {"intent": "unknown", "fields": {}, "filters": {}, "confidence": 0.0}
        ...
        if "confidence" not in data:
            data["confidence"] = 0.75  # default mid confidence

        return data

    except Exception as e:
        print("GPT intent extraction failed:", e)
        return {"intent": "unknown", "fields": {}, "filters": {}, "confidence": 0.0}


def parse_nl_to_payload(text: str) -> Dict[str, Any]:
    """
    Primary entry: converts a natural-language user message into model-ready fields.
    1) Try GPT for structured extraction.
    2) Fallback to regex + keyword parsing if GPT fails.
    """
    ...
    if m:
        try:
            payload["amount"] = float(m.group(1).replace(",", ""))
        except Exception:
            pass

    # Basic keyword-based recovery
    if "dc" in t or "washington" in t:
        payload["merchant_state"] = "DC"
    if "grocery" in t or "supermarket" in t:
        payload["merchant_type"] = "Grocery Stores, Supermarkets"
    if "restaurant" in t or "food" in t:
        payload["merchant_type"] = "Eating Places and Restaurants"
    if "chip" in t:
        payload["use_chip"] = "Yes"

    # Detect general fraud-related wording
    if any(w in t for w in ["fraud", "risky", "scam", "suspicious", "flag", "alert"]):
        intent = "score_single"
    else:
        intent = "unknown"
    ...
    return {
        ...
        "need_example": False,
        "example_type": "",
        "confidence": 0.4 if payload else 0.1,
    }
...
# Define file paths (these match your VS Code folder)
KB_FEATURES_PATH = "./fraud_knowledge_base_features.csv"
KB_PATTERNS_PATH = "./fraud_knowledge_base_patterns.md"

# Load KB files
try:
    KB_DF = pd.read_csv(KB_FEATURES_PATH)
    print(f"✅ Loaded feature KB: {len(KB_DF)} entries")
except Exception as e:
    KB_DF = pd.DataFrame()
    print("⚠️ Could not load feature KB:", e)

try:
    with open(KB_PATTERNS_PATH, "r", encoding="utf-8") as f:
        KB_PATTERNS = f.read()
    print("✅ Loaded pattern KB (Markdown)")
except Exception as e:
    KB_PATTERNS = ""
    print("⚠️ Could not load pattern KB:", e)


# --- Hybrid KB Search Function ---
def kb_search(query: str, k: int = 5) -> list:
    """
    Hybrid RAG search combining feature KB (CSV) and pattern KB (Markdown).
    """
    results = []
    q = query.lower().strip()

    # Search features KB (CSV)
    if not KB_DF.empty:
        for _, row in KB_DF.iterrows():
            ...
            if q in title.lower() or q in desc.lower():
                results.append({
                    ...
                    "title": title,
                    "text": desc,
                    "source": "fraud_knowledge_base_features.csv"
                })

    # Search pattern KB (Markdown)
    if KB_PATTERNS:
        sections = KB_PATTERNS.split("## ")
        for section in sections:
            if q in section.lower():
                ...
                results.append({
                    ...
                    "title": title,
                    "text": text,
                    "source": "fraud_knowledge_base_patterns.md"
                })
...
def _fuzzy_match(a: str, b: str, threshold=0.65) -> bool:
...
def kb_search(query: str, k: int = 5) -> list[dict]:
    ...
    if not KB_DF.empty:
        for _, row in KB_DF.iterrows():
            title = str(row.get("title", ""))
            desc = str(row.get("description", ""))
            if q in title.lower() or q in desc.lower() or _fuzzy_match(q, title) or _fuzzy_match(q, desc):
                results.append({
                    "kb_type": "feature",
                    "title": title,
                    "text": desc,
    ...
    if KB_PATTERNS:
        ...
        for section in sections:
            if not section.strip():
                continue
            if q in section.lower() or _fuzzy_match(q, section):
                title = section.split("\n")[0].strip()
                text = "\n".join(section.split("\n")[1:]).strip()
                results.append({
                    "kb_type": "pattern",
